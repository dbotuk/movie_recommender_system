...
1. **Step 1: select a user**
2. **Step 2: generate initial candidate recommendations using the SVD model**. We use the trained SVD model to predict ratings for all movies that the selected user has not yet rated. Then, we select the top-N movies with the highest predicted ratings as initial candidates.
3. **Step 3: re-rank the candidates using the GradientBoosting model**. For each of the top-N candidate movies, we create feature vectors that include the predicted rating from the SVD model and other relevant features (like user demographics and movie attributes). Then, we use the trained GradientBoosting model to predict a ranking score for each candidate.
4. **Step 4: display the final recommendations**. We sort the candidate movies by the ranking score generated by the GradientBoosting model and display the top-ranked movies as the final recommendations for the user.


The performance of the developed system was evaluated using three key metrics: Mean Average Precision (MAP), Mean Reciprocal Rank (MRR), and Normalized Discounted Cumulative Gain (NDCG). The results showed high scores across all three metrics, indicating that the system was effective in providing relevant and well-ranked recommendations.
